...
TEST(Base64Encode, EncodesThreeBytesZeros)
{
    ...
    auto const actual{base64::encode({begin(input), end(input)})};
    ASSERT_EQ(expected, actual);
}
...
TEST(Base64Encode, EncodesFourBytes)
{
    ...
    auto actual{base64::encode({begin(input), end(input)})};
    ASSERT_EQ(expected, actual);
}
...
TEST(Base64Decode, FailDecodeOneString)
{
    ...
    auto const actual{base64::decode(input)};
...
TEST(Base64Decode, DecodesEmptyString)
{
    ...
    auto const actual{base64::decode("")};
